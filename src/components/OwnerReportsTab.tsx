...
export const OwnerReportsTab: React.FC<OwnerReportsTabProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'sales_analytics' | 'calc_history' | 'market_credit'>('sales_analytics');
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter((t) => {
      ...
      const matchesOutlet =
        ...
        t.storeId === selectedOutletFilter ||
        (!t.storeId && (selectedOutletFilter === (storeSettings.activeStoreId || 'store-1') || selectedOutletFilter === outlets[0]?.id));
  ...
  const outletBreakdown = useMemo(() => {
    return outlets.map((outlet) => {
      const outletTxs = transactions.filter((t) => {
        ...
        const isThisStore = t.storeId === outlet.id || (!t.storeId && (outlet.isPrimary || outlet.id === outlets[0]?.id));
  ...
  }, [outlets, transactions, selectedPeriod, dateRanges]);
