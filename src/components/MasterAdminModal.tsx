...
export const MasterAdminModal: React.FC<MasterAdminModalProps> = ({
  ...
  // Poll central DB status and audit logs on mount
  useEffect(() => {
    if (!isOpen) return;
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                {outletsList
                  ...
                  .map((outlet) => {
                    ...
                    const todaySales = outletTodayTxs
                      .filter((t) => t.type === 'sale')
                      .reduce((sum, t) => sum + (t.amount || 0), 0);
                    const cashSales = outletTodayTxs
                      .filter((t) => t.type === 'sale' && t.paymentMode === 'cash')
                      .reduce((sum, t) => sum + (t.amount || 0), 0);
                    const upiSales = outletTodayTxs
                    ...
                    const assignedStaff = currentStaff.filter(
                      (s) => !s.assignedOutletIds || s.assignedOutletIds.length === 0 || s.assignedOutletIds.includes(outlet.id)
                    );
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {outlet.shortcutName && (
                                  <span className="px-2 py-0.5 bg-amber-500/20 text-amber-300 text-[10px] font-extrabold rounded-md border border-amber-500/30 uppercase">
                                    {outlet.shortcutName}
                                  </span>
                                )}
                                {outlet.isDefault && (
                        ...
                        {/* Key Performance Metrics for this Outlet */}
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                          <div className="bg-slate-800/70 border border-slate-700/60 rounded-xl p-2.5">
                            ...
                            <span className="text-[10px] text-slate-500 block">{outletTodayTxs.length} bills today</span>
                          </div>
                          ...
                          <div className="bg-slate-800/70 border border-slate-700/60 rounded-xl p-2.5">
                            ...
                            <span className="text-[10px] text-slate-500 block">Drawer physical</span>
                          </div>
                          ...
                          <div className="bg-slate-800/70 border border-slate-700/60 rounded-xl p-2.5">
                            ...
                            <span className="text-[10px] text-slate-500 block">Digital account</span>
                          </div>

                          <div className="bg-slate-800/70 border border-slate-700/60 rounded-xl p-2.5">
                            <span className="text-[10px] uppercase font-bold text-slate-400 block">Udhaar Ledger</span>
                            <span className="text-base font-black text-rose-400 font-mono">
                              ₹{outletUdhaarTotal.toLocaleString('en-IN')}
                            </span>
                            <span className="text-[10px] text-slate-500 block">{outletCustomers.length} cust. pending</span>
                          </div>
                        </div>
