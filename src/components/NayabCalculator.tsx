    ...
    <div className="w-full max-w-lg mx-auto space-y-2.5">
      ...
      <div className="w-full bg-gradient-to-b from-slate-900 via-slate-925 to-slate-950 rounded-2xl sm:rounded-3xl p-3 sm:p-4 md:p-5 shadow-2xl border border-slate-800 ring-1 ring-slate-800/80">
        ...
        <div className="mt-2 bg-stone-900 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-2xl border-2 border-stone-800 shadow-inner relative overflow-hidden flex flex-col justify-between">
          ...
          {/* Main Area: Constant Total Digit Size - Does NOT change or shrink */}
          <div className="relative flex flex-col justify-center my-1 sm:my-1.5 overflow-hidden w-full">
            <div
              ref={formulaDisplayRef}
              className="w-full flex items-center justify-end overflow-x-auto scrollbar-none py-1"
            >
              <div
                className="font-['Share_Tech_Mono',monospace] text-5xl sm:text-6xl md:text-7xl font-black tracking-tight text-emerald-300 drop-shadow-[0_0_16px_rgba(52,211,153,0.45)] select-all leading-tight text-right whitespace-nowrap"
              >
                {isCompleted && evaluatedResult !== null ? (
                ...
                {isCompleted && (
                  <span className="text-emerald-400/80 ml-1 font-sans font-bold text-3xl sm:text-4xl">=</span>
                )}
                {!isCompleted && (
