...
export const StoreSettingsModal: React.FC<StoreSettingsModalProps> = ({
  ...
  const handleVerifyOwnerPin = (e: React.FormEvent) => {
    ...
    if (cleanPin !== 'nayab@q6' && ownerStaffAccount?.pin && cleanPin !== ownerStaffAccount.pin) {
  ...
  const handleSaveAll = () => {
    // If not owner, do not allow staff to modify staff accounts, roles or stores
      ...
      <div className="w-full max-w-3xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center gap-1 p-2.5 bg-slate-950/60 border-b border-slate-800 overflow-x-auto text-xs font-semibold">
          ...
          >
            ...
            <span>3. Multi-Store Outlets ({currentStores.length}) {!isOwnerPermissionGranted && '(Admin Only)'}</span>
          </button>
          ...
          >
            ...
            <span>4. Staff Accounts {!isOwnerPermissionGranted && '(Owner Only)'}</span>
          </button>
          ...
          >
            ...
            <span>8. Owner Reports (Daily, Weekly, Monthly, Yearly)</span>
          </button>
          ...
          >
            ...
            <span>9. Free APIs & Cloud Status</span>
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 text-xs">
          ...
          {activeTab === 'upi' && (
            <div className="space-y-4">
              {!isOwnerPermissionGranted ? (
                /* Owner Lock Notice for UPI Scanner Settings */
                <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="font-bold text-white text-sm">Owner-Only Access: Merchant UPI ID & Scanner</h4>
                      <p className="text-slate-300 text-xs mt-0.5">
                        Only the store owner (<span className="text-amber-300 font-semibold">{ownerStaffAccount.name}</span>) has permission to edit or change the merchant bank UPI ID and QR code scanner configuration.
                      </p>
                    </div>
                  </div>

                  {/* Owner PIN Verification Form */}
                  <form onSubmit={handleVerifyOwnerPin} className="pt-2 border-t border-amber-500/20 flex flex-wrap items-center gap-2">
                    <span className="text-xs text-slate-300 font-medium">Owner PIN to Unlock:</span>
                    <input
                      type="password"
                      maxLength={12}
                      ...
                      className="px-3 py-1.5 bg-slate-900 border border-amber-500/40 rounded-xl text-xs text-white font-mono focus:outline-none focus:border-amber-400"
                    />
                    <button
                      type="submit"
                      className="px-3.5 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-xl text-xs transition-colors flex items-center gap-1 cursor-pointer"
                    >
                      <Lock className="w-3.5 h-3.5" />
                      <span>Unlock Editing</span>
                    </button>
                    {unlockPinError && (
                      <span className="text-rose-400 text-xs font-semibold">{unlockPinError}</span>
                    )}
                  </form>
                </div>
              ) : (
                ...
                <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold">
                    ...
                    <span>Owner Permissions Granted — You can modify UPI payment routing and scanner settings.</span>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className={`w-full border rounded-xl px-3.5 py-2.5 text-white font-mono text-sm focus:outline-none ${
                            !isOwnerPermissionGranted
                      ...
                      {!isOwnerPermissionGranted && (
                        <p className="text-[11px] text-amber-400/80 mt-1">
                          🔒 Read-only mode. Enter owner PIN above to modify this UPI ID.
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full border rounded-xl px-3 py-2 text-white ${
                          !isOwnerPermissionGranted
                            ? 'bg-slate-900/60 border-slate-800 text-slate-400 cursor-not-allowed'
          ...
          {/* TAB 3: MULTI-STORE OUTLETS - ADMIN / OWNER ONLY */}
          {activeTab === 'stores' && (
            <div className="space-y-4">
              {!isOwnerPermissionGranted ? (
                ...
                <div className="p-6 bg-slate-900/90 border-2 border-amber-500/50 rounded-2xl text-center space-y-4 max-w-md mx-auto my-6 shadow-2xl">
                  ...
                  <div className="space-y-1">
                    <h4 className="text-base font-extrabold text-white">Owner / Admin Access Restricted</h4>
                    <p className="text-xs text-amber-200/80 leading-relaxed">
                      Managing multiple store outlets and branches is strictly restricted to the Store Owner / Admin (<span className="text-amber-300 font-bold">{ownerStaffAccount.name}</span>). Please enter the Owner PIN below to unlock.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleVerifyOwnerPin} className="space-y-3 pt-1">
                    <div className="text-left">
                      <label className="text-xs font-semibold text-slate-300 block mb-1">
                        Enter Owner PIN to unlock store management:
                      </label>
                      <input
                        type="password"
                        maxLength={30}
                    ...
                    >
                      <ShieldCheck className="w-4 h-4" />
                      <span>Verify Owner PIN & Unlock Stores</span>
                    </button>
                  </form>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-bold text-white text-sm">Multiple Store Outlets & Branches</h4>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">
                          {currentStores.length} {currentStores.length === 1 ? 'Store' : 'Stores'} Registered
                        </span>
                      </div>
                      <p className="text-slate-400 text-xs mt-0.5">
                        Admin option to create, manage, and seamlessly switch between multiple stores or branches. Active store controls receipt headers and QR UPI payments.
                      </p>
                    </div>
                    ...
                    {!showAddStoreModal && (
                      <button
                        ...
                        className="px-3.5 py-2 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-md cursor-pointer flex-shrink-0"
                      >
                        <Plus className="w-4 h-4" />
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            {/* Details List */}
                            <div className="space-y-1.5 my-3 text-[11px] bg-slate-950/50 p-2.5 rounded-xl border border-slate-800/80">
                              ...
                              {store.address && (
                                <div className="flex items-start gap-2 text-slate-300">
                                  ...
                                  <span className="line-clamp-2 text-slate-400">{store.address}</span>
                                </div>
                              )}
          ...
          {/* TAB 4: STAFF ACCOUNTS WITH PASSWORD - OWNER RESTRICTED */}
          {activeTab === 'staff' && (
            <div className="space-y-4">
              {!isOwnerPermissionGranted ? (
                /* Non-owner staff locked out banner */
