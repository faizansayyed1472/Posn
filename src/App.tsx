...
import { fetchCentralSnapshot, pushCentralDelta, saveCentralProduct, deleteCentralProduct } from './utils/centralSync';
import {
...
export default function App() {
  ...
  // Central Database Cloud Sync (Load initial snapshot from server & verify Firestore connection)
  useEffect(() => {
    let isMounted = true;
    testFirestoreConnection().catch((err) => {
      console.info('Firestore boot connection status:', err);
    });
  ...
  // Compute Today's sales total scoped to current active counter outlet
  ...
  const todaySalesTotal = activeOutletTransactions
    .filter((t) => !t.voided && t.type === 'sale' && new Date(t.timestamp).getTime() >= startOfToday)
    .reduce((sum, t) => sum + t.amount, 0);

  const todayOnlineSalesTotal = activeOutletTransactions
    .filter(
      (t) =>
        !t.voided &&
        t.type === 'sale' &&
  ...
  const todayUdhaarSalesTotal = activeOutletTransactions
    .filter(
      (t) =>
        !t.voided &&
        t.type === 'sale' &&
  ...
  const todayExpensesTotal = activeOutletTransactions
